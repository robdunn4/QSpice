...
ActionButton::ActionButton(QAction* action, QWidget* parent) : QPushButton(parent)
{
  Q_ASSERT(action);
  setAction(action);
}
...
{
  // if there is already an action associated with the button, remove all connections
