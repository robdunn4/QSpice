 ...
 *
 * This class represents a QPushButton extension that can be connected to an action and that
 * configures itself depending on the status of the action.  When the action changes its state,
 * the button reflects such changes, and when the button is clicked the action is triggered.
 * @note This code is derived from https://wiki.qt.io/PushButton_Based_On_Action.
 */
...
public:
   ...
   * are connected together so that when the action is changed the button is updated and when the button is clicked the
   * action is triggered.
   *
