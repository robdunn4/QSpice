...
public:
  ...
  ArgString(const char *cstr);
  ArgString(const ArgString &other);
  virtual ~ArgString() = default;
  ...
  std::string        toAsciiString() const; // strips high-bit bytes

  ArgString       &splitQuoted(ArgString &secondPart);
  ArgString        quote() const;
  static ArgString quote(const ArgString &str) { return str.quote(); }
};
