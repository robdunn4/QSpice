//------------------------------------------------------------------------------
// CB12_Client.cpp -- Example client DLL code for C-Block Basics #12.
//------------------------------------------------------------------------------
...
#undef IN    // avoid macro conflicts
#undef OUT   // avoid macro conflicts
...
#define PORT_GETPORTNBR  1032   // get: get server port number
 ...
 */
extern "C" __declspec(dllexport) void cb12_client(
  ...
  if (!inst) { /* begin one-time initialization section */
    ...
    // get port number from server; display in output window for reference
    ConfigureBuffer(inst->buffer, PORT_GETPORTNBR);
    SocketSend(inst->ConnectSocket, inst->buffer, 4);
    if (SocketRecv(inst->ConnectSocket, inst->buffer, 4) != 4)
      EXIT("PORT_GETPORTNBR message returned invalid length.");

    unsigned int *pPortNbr = (unsigned int *)inst->buffer;
    Display("Connected to server at %s on server port %u\n", server, *pPortNbr);
