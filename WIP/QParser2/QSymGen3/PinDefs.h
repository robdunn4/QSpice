...
protected:
  static constexpr char             typeChars[]{"IOBXVGK"};
  static constexpr std::string_view typeNames[]{
      "Input", "Output", "BiDir", "Skip", "VDD", "GND", "SimClock"};
};
...
public:
  ...
  std::string description{"Symbol generated with QSymGen3"};
  int         biDirPinCnt = 0;
  int         inPinCnt    = 0;
  int         outPinCnt   = 0;
  int         skipPinCnt  = 0; // excluded from totPinCnt
  int         vddPinCnt   = 0;
  int         gndPinCnt   = 0;
  int         clockPinCnt = 0;
  ...
  std::string clockPinName{""}; // name of the single C pin; set during parse
