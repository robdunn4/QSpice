...
The prior code was sufficient for parsing an existing QSpice schematic or symbol file.  However, it was poorly planned (and inadequate) for generating new schematic/symbol files entirely from code.  This release attempts to rectify that shortcoming.  However, that required a major rework and prior sample code will no longer work.
...
* [QSymGen2](./QSymGen2/) &mdash; QSpice symbol generator for the *QMdbSim* micro-controller simulator project.  Similar to QSymGen but recoded from scratch.  Adds support for GPIO pins. **Not Complete**
