...
// TODO:  Revisit parse() static methods.  Originally intended to be called by
// constructors, but that seems a little clunky.
...
ArgString::ArgString(const char *cstr) : value(cstr) {}
...
// split special cases (e.g., ItemNet and ItemPin) where there may be two
// trailing quoted strings, one of which is optional.  the tokenizer will not
// split these into separate arguments, so we need to do it for the special
// cases.
ArgString &ArgString::splitQuoted(ArgString &secondPart) {
  auto pos = value.find(' ');
  if (pos != std::string_view::npos) {
    secondPart = std::string(value.substr(pos + 1));
    value      = std::string(value.substr(0, pos));
  }
  return *this;
}

// when constructing ItemBase-derived classes via constructors in code, the
// developer should not be required to include quotes around string arguments.
// ArgString &ArgString::quote() {
//  value = "\"" + value + "\"";
//  return *this;
ArgString ArgString::quote() const {
...
// ============================================================================
// ArgString implementation (adds quoting + ASCII cleanup)
// ============================================================================

//// TODO:  Revert back to only ArgString base class; for now, undo quoting
//// std::string ArgString::toString() const { return "\"" + value + "\""; }
// std::string ArgString::toString() const { return value; }
//
// std::string ArgString::toAsciiString() const {
